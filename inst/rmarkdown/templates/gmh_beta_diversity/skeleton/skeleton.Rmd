...
You can run a PERMANOVA for each pairwise combination of groups in a variable we use the `pairwise.adonis2` from the package \code{pairwiseAdonis}.
The function can only test one variable at the time, but does identify differences between each pair of groups.

```{r, eval = TRUE}

# Perform pairwise test
pairwise.adonis2(dist.used ~ get(VAR), data = mdat)
